...
| `api_pack_purchases` — the API pack invoice/recovery ledger | Stripe ids, operation ids, quantities and timestamps. No documents or people. Kept with the subscription, including unfinished operations: reconciliation can still void an unpaid invoice or identify a paid invoice needing operator review after cancellation. Both the association and foreign key refuse deletion of a subscription with this ledger. |
...
| `operator_events` — our own log of support access to the account | Kept, and **still attached to the account row** (which is now the tombstone), because this is the record of what *we* did to the account — every support session one of our operators opened, every change they made, every door they were refused at, and the deletion itself. It is not the customer's record to take away: it is the record of our access to them, and the Privacy Policy already tells them it outlives the account. **But the person is scrubbed out of it first.** When an operator opens a support session we write down which administrator they signed in as, and that is an email address; on a purge, any email address or personal name stored in an event's details is replaced with `[purged]`. What stays is the audit — what was done, by which operator, why (the written reason they gave), when, and from what address *they* connected — plus every id, count and outcome. So the log still answers "what did we ever do to that account", and no longer says who the customer was. |
...
| `api_pack_purchases` | **keep** | Invoice and recovery history, including any unresolved money operation (see above). |
| `verified_documents` | **untouched** | Public verification (see above). |
| `accounts` | **tombstone** | Renamed, stamped, kept. |
| `operator_events` | **keep, scrubbed** | Our log of support access to the account (see above). The rows and their `account_id` stay; email addresses and personal names inside `details` become `[purged]`. |
