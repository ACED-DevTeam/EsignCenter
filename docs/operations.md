...
| `FORCE_SSL` | Required (exactly `true`) | `lib/docuseal.rb` (links become `https`), production SSL redirect, `lib/production_readiness.rb` | **Production refuses to boot** when it is missing or has any other value. HTTPS redirects, secure cookies and proxy SSL handling stay enabled unconditionally in the production environment. |
...
| `SMTP_USERNAME`, `SMTP_PASSWORD` | Required with `SMTP_ADDRESS` | `lib/mail_configs.rb`, `config/initializers/email_delivery.rb` | Falls back to `POSTMARK_API_TOKEN` (below). **Production refuses to boot** unless the pair or that token is present. For Postmark, the EsignCenter server token goes in both. |
...
| `SMTP_DOMAIN`, `SMTP_AUTHENTICATION`, `SMTP_ENABLE_STARTTLS`, `SMTP_ENABLE_SSL`, `SMTP_ENABLE_TLS`, `SMTP_SSL_VERIFY`, `SMTP_OPEN_TIMEOUT`, `SMTP_READ_TIMEOUT` | Optional | `lib/mail_configs.rb`, `lib/production_readiness.rb` | Tuning for non-Postmark servers. Defaults: STARTTLS on, certificate verification on, 15 s open / 25 s read timeouts. Production refuses to boot if certificate verification is disabled, or if STARTTLS is disabled without direct SSL/TLS enabled. Leave unset for Postmark. Per-account pinned SMTP retains its existing settings and must pass the deployment parity check. |
| `EMAIL_DELIVERY_MODE` | Optional | `lib/mail_configs.rb`, `config/initializers/email_delivery.rb` | Defaults to `smtp` in production and `test` elsewhere. Production refuses to boot if it is explicitly set to `test` or any other value; test mode remains available in local development and the test suite. |
| `TIMESERVER_URL` | **Required** | `lib/docuseal.rb`, `lib/accounts.rb`, `config/initializers/timestamp_server_guard.rb` | The trusted timestamp authority (the approved DigiCert endpoint is `http://timestamp.digicert.com`) stamped into every signed PDF; several absolute HTTP(S) URLs may be listed comma-separated and every one is tried in turn. **Boot refuses to start** in production when the value is missing or malformed. If no authority answers at signing time the signing job **fails loudly** — the app reports it once and Sidekiq retries the job — instead of embedding a locally generated time that only looked trusted (section 8.4). |
