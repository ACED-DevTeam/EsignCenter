...
mortgage app) using **your own EsignCenter service**.

Your trusted apps use automatically provisioned **internal accounts**, which do not
need a subscription and are not subject to customer plan quotas. Public SaaS
customers use separate accounts; their API and embedding access requires the paid
plan. Running the same service does not mix those accounts or their documents.

The API supports reusable templates, PDFs generated by your app, signing sessions,
embedded template editing, and read-only previews.
...
These workspaces are **internal accounts**: they do not need a customer signup,
email confirmation, Stripe subscription, or paid seats, and they bypass customer
plan quotas. The admin user is created automatically with a random password; your
app stores the returned API token and webhook signing secret in its backend.
Use a distinct stable email and idempotency key for each app/workspace so their
documents remain isolated. The provisioning secret is an operator credential;
never give it to a public customer or put it in frontend code.

Public customers sign up separately and use the free or paid plan. Their
accounts cannot promote themselves to internal accounts or see your apps'
workspaces.
