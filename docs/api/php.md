...
>
> **Whose library this is.** The examples below use DocuSeal's own PHP client
> (the `\Docuseal\Api` class), pointed at your EsignCenter address. DocuSeal
> publishes and maintains that library. EsignCenter is an independent fork of
> DocuSeal and is not affiliated with, endorsed by or supported by DocuSeal, so
> questions about the library itself go to its maintainers. Every operation here
> is also a plain HTTPS request with an `X-Auth-Token` header — see
> [shell.md](shell.md) for the curl form if you would rather not depend on the
> library.
...
```php
$docuseal = new \Docuseal\Api('API_KEY', 'https://your-instance.example.com/api');
