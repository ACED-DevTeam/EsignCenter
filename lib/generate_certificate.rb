...
module GenerateCertificate
  ...
  def generate_root_ca(name)
    ...
    cert.subject = OpenSSL::X509::Name.parse("/C=US/O=#{name}/CN=#{name} Root CA")
  ...
  def generate_sub_ca(name, root_ca_cert, root_ca_key)
    ...
    cert = OpenSSL::X509::Certificate.new
    cert.subject = OpenSSL::X509::Name.parse("/C=US/O=#{name}/CN=#{name} Sub-CA")
  ...
  def generate_certificate(name, ca_cert, ca_key)
    ...
    cert = OpenSSL::X509::Certificate.new
    cert.subject = OpenSSL::X509::Name.parse("/C=US/O=#{name}/CN=#{name}")
