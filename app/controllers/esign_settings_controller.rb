...
# Certificate management is a platform-operator surface (Session 4): every
# customer signs with the platform certificate, and an internal account's own
# row is managed from the console, not in-app. Only `show` — which also carries
# the signing preferences every admin owns — is open to account admins.
class EsignSettingsController < ApplicationController
  DEFAULT_CERT_NAME = 'EsignCenter Self-Host Autogenerated'
  LEGACY_DEFAULT_CERT_NAME = 'DocuSeal Self-Host Autogenerated'
  ...
  # Prepended so a non-operator meets the 404 before anything else answers.
  prepend_before_action :require_operator_access!, only: %i[new create update destroy]
  ...
  def create
    ...
    if (@encrypted_config.value && @encrypted_config.value['custom']&.any? { |e| e['name'] == @cert_record.name }) ||
       @cert_record.name.in?([DEFAULT_CERT_NAME, LEGACY_DEFAULT_CERT_NAME])
