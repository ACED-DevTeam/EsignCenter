...
RSpec.describe 'Seats and invitations', type: :request do
  ...
  describe 'releasing a seat' do
    ...
    it 'never goes below the people who are actually there' do
      ...
      expect(BillingLifecycle.release_seats!(row)).to be(false)
  ...
  describe 'an address that already has an account of its own' do
    ...
    it 'says in the email what accepting will do', sidekiq: :inline do
      ...
      expect(body_of(mail)).to include(other_account.name)
      expect(body_of(mail)).to include(account.name)
    end
  ...
  # The confirm request Stripe must have received: the new seat count, the
  # proration that invoices it today, and the safety catch that refuses to
  # leave a half-changed subscription behind.
  def seat_purchase_body?(request)
    request.body.include?('quantity%5D=2') &&
      request.body.include?('proration_behavior=always_invoice') &&
      request.body.include?('payment_behavior=pending_if_incomplete') &&
      request.headers['Idempotency-Key'].to_s.start_with?('seat-add:')
  end
